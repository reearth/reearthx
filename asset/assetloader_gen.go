...
type AssetLoaderConfig struct {
	...
	Fetch func(keys []AssetID) ([]*Asset, []error)
...
func (l *AssetLoader) Load(key AssetID) (*Asset, error) {
...
func (l *AssetLoader) LoadThunk(key AssetID) func() (*Asset, error) {
	l.mu.Lock()
...
func (l *AssetLoader) LoadAll(keys []AssetID) ([]*Asset, []error) {
...
// This method should be used if you want one goroutine to make requests to many
// different data loaders without blocking until the thunk is called.
func (l *AssetLoader) LoadAllThunk(keys []AssetID) func() ([]*Asset, []error) {
	results := make([]func() (*Asset, error), len(keys))
	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}
...
func (l *AssetLoader) Prime(key AssetID, value *Asset) bool {
	l.mu.Lock()
	var found bool
...
func (l *AssetLoader) Clear(key AssetID) {
	l.mu.Lock()
	delete(l.cache, key)
	l.mu.Unlock()
}

func (l *AssetLoader) unsafeSet(key AssetID, value *Asset) {
	if l.cache == nil {
		l.cache = map[AssetID]*Asset{}
	}
	l.cache[key] = value
}
...
func (b *assetLoaderBatch) keyIndex(l *AssetLoader, key AssetID) int {
