...
func Test_AreItemsReferenced(t *testing.T) {
	...
	f1 := schema.NewField(schema.NewReference(mid2, sid1, fid2.Ref(), cf1).TypeProperty()).
		ID(fid1).
		Key(id.RandomKey()).
		MustBuild()
	f2 := schema.NewField(schema.NewReference(mid1, sid2, fid1.Ref(), cf2).TypeProperty()).
		ID(fid2).
		Key(id.RandomKey()).
		MustBuild()
	f3 := schema.NewField(schema.NewReference(mid3, sid3, nil, nil).TypeProperty()).
		ID(fid3).
		Key(id.RandomKey()).
		MustBuild()
	s1 := schema.New().
		...
		Project(prj.ID()).
		Fields(schema.FieldList{f1}).
		MustBuild()
	s2 := schema.New().
		ID(sid2).
		Workspace(accountdomain.NewWorkspaceID()).
		Project(prj.ID()).
		Fields(schema.FieldList{f2}).
		MustBuild()
	s3 := schema.New().
		ID(sid3).
		Workspace(accountdomain.NewWorkspaceID()).
		Project(prj.ID()).
		Fields(schema.FieldList{f3}).
		MustBuild()
