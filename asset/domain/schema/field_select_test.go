...
func TestNewSelect(t *testing.T) {
	assert.Equal(
		t,
		&FieldSelect{values: []string{"a", "b"}},
		NewSelect([]string{"a", "b", " a", "b"}),
	)
}
...
func TestFieldSelect_Validate(t *testing.T) {
	assert.NoError(
		t,
		(&FieldSelect{values: []string{"aaa"}}).Validate(value.TypeSelect.Value("aaa")),
	)
	assert.Equal(
		t,
		ErrInvalidValue,
		(&FieldSelect{values: []string{"aa"}}).Validate(value.TypeSelect.Value("aaa")),
	)
