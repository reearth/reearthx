...
func TestNewString(t *testing.T) {
	assert.Equal(
		t,
		&FieldString{t: value.TypeText, maxLength: lo.ToPtr(1)},
		NewString(value.TypeText, lo.ToPtr(1)),
	)
}
...
func TestFieldString_Clone(t *testing.T) {
	...
	assert.Equal(
		t,
		&FieldString{t: value.TypeText, maxLength: lo.ToPtr(1)},
		(&FieldString{t: value.TypeText, maxLength: lo.ToPtr(1)}).Clone(),
	)
}
...
func TestFieldString_Validate(t *testing.T) {
	assert.ErrorContains(
		t,
		(&FieldString{t: value.TypeText, maxLength: lo.ToPtr(1)}).Validate(
			value.TypeText.Value("aaa"),
		),
	...
	assert.ErrorContains(
		t,
		(&FieldString{t: value.TypeText, maxLength: lo.ToPtr(1)}).Validate(
			value.TypeText.Value("ああ"),
		),
		"value has 2 characters, but it sholud be shorter than 1 characters",
	)
	assert.NoError(
		t,
		(&FieldString{t: value.TypeText, maxLength: lo.ToPtr(4)}).Validate(
			value.TypeText.Value("あ"),
		),
	)
	...
	assert.Equal(
		t,
		ErrInvalidValue,
		(&FieldString{t: value.TypeText}).Validate(value.TypeNumber.Value(1)),
	)
}
