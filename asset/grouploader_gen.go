...
type GroupLoaderConfig struct {
	...
	Fetch func(keys []GroupID) ([]*Group, []error)
...
func (l *GroupLoader) Load(key GroupID) (*Group, error) {
...
func (l *GroupLoader) LoadThunk(key GroupID) func() (*Group, error) {
	l.mu.Lock()
	if it, ok := l.cache[key]; ok {
		l.mu.Unlock()
		return func() (*Group, error) {
			return it, nil
		}
	}
	if l.batch == nil {
...
func (l *GroupLoader) LoadAll(keys []GroupID) ([]*Group, []error) {
	results := make([]func() (*Group, error), len(keys))
...
// This method should be used if you want one goroutine to make requests to many
// different data loaders without blocking until the thunk is called.
func (l *GroupLoader) LoadAllThunk(keys []GroupID) func() ([]*Group, []error) {
	results := make([]func() (*Group, error), len(keys))
	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}
...
func (l *GroupLoader) Prime(key GroupID, value *Group) bool {
	l.mu.Lock()
	var found bool
...
func (l *GroupLoader) Clear(key GroupID) {
	l.mu.Lock()
	delete(l.cache, key)
	l.mu.Unlock()
}

func (l *GroupLoader) unsafeSet(key GroupID, value *Group) {
	if l.cache == nil {
		l.cache = map[GroupID]*Group{}
	}
	l.cache[key] = value
}
...
func (b *groupLoaderBatch) keyIndex(l *GroupLoader, key GroupID) int {
