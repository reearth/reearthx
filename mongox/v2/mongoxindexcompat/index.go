...
func Indexes(ctx context.Context, c *mongo.Collection, keys, uniqueKeys []string) ([]string, []string, error) {
	...
	if len(newIndexes) > 0 {
		if _, err := c.Indexes().CreateMany(ctx, newIndexes); err != nil {
			return nil, nil, err
		}
	}
...
func (i indexDocument) Model() mongo.IndexModel {
	var o *options.IndexOptionsBuilder
	if i.Unique {
