...
func (c *Collection) Paginate(ctx context.Context, rawFilter any, s *usecasex.Sort, p *usecasex.Pagination, consumer Consumer, opts ...options.Lister[options.FindOptions]) (*usecasex.PageInfo, error) {
...
func (c *Collection) PaginateAggregation(ctx context.Context, pipeline []any, s *usecasex.Sort, p *usecasex.Pagination, consumer Consumer, opts ...options.Lister[options.AggregateOptions]) (*usecasex.PageInfo, error) {
	if p == nil || p.Cursor == nil && p.Offset == nil {
		return nil, nil
	}
	...
	cursor, err := c.collection.Aggregate(ctx, pPipeline, append([]options.Lister[options.AggregateOptions]{pOpts}, opts...)...)
	if err != nil {
...
func (c *Collection) aggregateFilter(ctx context.Context, p usecasex.Pagination, s *usecasex.Sort) ([]any, options.Lister[options.AggregateOptions], error) {
...
func aggregateOptionsFromPagination(_ usecasex.Pagination, _ *usecasex.Sort) options.Lister[options.AggregateOptions] {
...
func (c *Collection) PaginateProject(ctx context.Context, rawFilter any, s *usecasex.Sort, p *usecasex.Pagination, consumer Consumer, opts ...options.Lister[options.FindOptions]) (*usecasex.PageInfo, error) {
	if p == nil || (p.Cursor == nil && p.Offset == nil) {
		return nil, nil
	}
...
func (c *Collection) paginate(ctx context.Context, rawFilter any, s *usecasex.Sort, p *usecasex.Pagination, filter any, consumer Consumer, opts []options.Lister[options.FindOptions]) (*usecasex.PageInfo, error) {
	...
	cursor, err := c.collection.Find(ctx, filter, append([]options.Lister[options.FindOptions]{findOpts}, opts...)...)
	if err != nil {
		return nil, rerror.ErrInternalByWithContext(ctx, fmt.Errorf("failed to find: %w", err))
	}
	defer func() {
